'''
@author: Fumiya Inaba 
Last edited: July 26, 2022 @ 1:01 AM 
File functionality: 
...
class ImageAnnotator(tk.Frame):
    def __init__(self, master, name):
        ...
        self.annotation_display_frame = tk.Frame(annotation_frame) # To display user defined classes
        instruction_frame = tk.Frame(annotation_frame) # To display instructions
        # ------------------------------------- User Input Bar ----------------------------------------- #
        self.v_var = tk.StringVar(self.annotation_display_frame) # Store class selection
        user_input_bar = tk.Frame(self.annotation_display_frame) # Frame to put in entry bar and add_class_button
    ...
    def select_image(self) -> None:
        '''
        Select an image to annotate and resize to 600x600 size if image is small. (Selected images should be 256x256 originally)
        '''
    ...
    def load_progress(self) -> list[dict]:
        '''
        Get data from the json file if it exists and load it into self.annotations
        '''
        ...
        # If file doesn't exist, return the template of data storage into self.annotations
        if not fileExists:
            return [{'imagefilename': self.image_name, 'annotation':[]}]
        else: # If file exists 
            with open(path_to_check, 'r') as f:
                f_contents = f.read()
                # If file exists and contains previous progress
                if f_contents != "":
                    ...
                    if self.image_name in already_annotated_files:
                        ...
                        return progress_made
                # If it's empty return template for json data storage
                else:
                    return [{'imagefilename': self.image_name, 'annotation':[]}]
    
    def render_previous_rect(self) -> None:
        '''
        Using information loaded into self.annotations, will render any previously made rectangle annotations on the 
        image so there are no duplicate annotations made between work sessions.
        '''
        # Load data using self.current_image_index (relative position in the self.annotations)
        annotations_made = self.annotations[self.current_image_index]['annotation']
        # Iterate through all rectangle annotations and render each to screen
        for annotate in annotations_made:
            coords = annotate['coordinates'] # Alias for where the coordinates are stored 
            middle_y = coords['y'] * self.ratio 
            middle_x = coords['x'] * self.ratio 
            rect_h = coords['height'] * self.ratio 
            rect_w = coords['width'] * self.ratio 
            # Get coordinates of the rectangle calculated using above parameters
            x1, y1, x2, y2 = self.get_rectangle_from_center((middle_x, middle_y, rect_w, rect_h))
            rect_index = annotations_made.index(annotate) # Tag to keep track of each previously made annotation
    ...
    def next_image(self) -> None:
        ...
        self.image_canvas.config(cursor = 'crosshair')
        # Update annotation count tracking parameters back to zero
        self.rect_count = 0 
        self.rect_label_count = 0
        # Load and render previously made annotations if it exists
        self.annotations = self.load_progress()
        self.render_previous_rect()
    ...
    def initiate_rectangle_drawing(self, event) -> None:
        '''
        Begins rectangle creation on click
        '''
    ...
    def draw_rectangle(self, event) -> None:
        '''
        Updates coordinates of rectangle created on click so user can visualize the resulting annotation
        '''
    ...
    def complete_rectangle(self, event) -> None:
        '''
        Completes the rectangle drawing and writes that information into self.annotation in it's respective location
        '''
        ...
        self.rect_count += 1
        # Delete cursor rectangle for user guidelines
        ...
        self.rect_label_count += 1
        # Add onto annotations for eventual export 
    ...
    def get_rectangle_center(self, rect_coords:tuple[int]) -> tuple[int]:
        '''
        Calculate the center point coordinate of the annotation along with the width and height of the annotation
        '''
    ...
    def get_rectangle_from_center(self, rect_coords:tuple[int]) -> tuple[int]:
        '''
        Given the center point (x,y) and width & height of a rectangle annotation, calculate the x1, y1, x2, y2 
        of the rectangle annotation
        '''
    ...
    def delete_recent_rect(self, events) -> None:
        '''
        Delete most recently made (most recently stored in self.annotations) rectangle annotation
        '''
        try:
        ...
        except IndexError: # If no annotations have been made, del annotation_dict[-1] will return IndexError
            pass

    # ------------------------------------- Class Annotation Methods -------------------------------------- #       
    def class_addition(self) -> None: 
        '''
        Add selectable class as the label to annotate objects in the image.
        '''
    ...
    def export_annotations(self) -> None:
        '''
        Export contents of self.annotations into a json file
        '''
        # Check if path exists for annotation file to go
        output_dir = os.getcwd()
        output_path = f'{output_dir}/Annotations'
        path_exists = os.path.exists(output_path)
        # If it doesn't exist, make the directory path
        if not path_exists:
            os.makedirs(output_path)
        # Open and overwrite all previous annotations
        ...
        # Give option for user to quit the application or to continue using app
        res = tk.messagebox.askquestion("Export", "Export success! You can now quit the application. \nTo quit, select yes. \nTo continue annotating, select no.")
        if res == 'yes':
            self.master.destroy()
